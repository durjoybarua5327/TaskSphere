...
export function GroupDetailClient({ group, initialMembers, initialRequests, initialTasks, initialTab = 'members', readOnly = false }: GroupDetailClientProps) {
        ...
        <div className="space-y-8 pb-20">
            ...
            <div className="bg-white border border-slate-100 rounded-[2rem] p-6 md:p-8 shadow-sm relative overflow-hidden">
                ...
                <div className="relative flex flex-col md:flex-row gap-6 items-start justify-between">
                    <div className="space-y-3 flex-1">
                        <div className="flex items-center gap-3 flex-wrap">
                            <div className="px-3 py-1 bg-emerald-50 text-emerald-600 rounded-full text-[9px] font-black uppercase tracking-widest flex items-center gap-2">
                                ...
                                {readOnly ? 'Student View' : (currentUserRole === 'top_admin' ? 'Top Admin Access' : 'Admin Access')}
                            </div>
                            ...
                            <div className="px-3 py-1 bg-blue-50 text-blue-600 rounded-full text-[9px] font-black uppercase tracking-widest flex items-center gap-2 cursor-pointer hover:bg-blue-100 transition-colors"
                                onClick={() => {
                                    setActiveTab('tasks');
                    ...
                    {/* Quick Add Member - Only if NOT readOnly */}
                    {!readOnly && (
                        <div className="w-full md:w-72 bg-slate-50 border border-slate-100 rounded-2xl p-4 space-y-3">
                            ...
                            <form onSubmit={handleAddMember} className="space-y-2">
                                ...
                                <button
                                    ...
                                    className="w-full group py-2 bg-slate-900 text-white rounded-xl font-black text-[9px] uppercase tracking-[0.2em] hover:bg-emerald-600 transition-all active:scale-95 flex items-center justify-center gap-2 disabled:opacity-50"
                                >
                                    {isAdding ? <Loader2 className="w-3 h-3 animate-spin" /> : <Plus className="w-3 h-3 transition-transform duration-500 group-hover:rotate-180" />}
                                    Add to Group
                                </button>
                            </form>
                        </div>
                    ...
                    <AnimatePresence mode="wait">
                        {activeTab === 'members' ? (
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {(readOnly || (!canPromote && !canRemove && !isMe)) && (
                            ...
                            >
                                <div className="flex items-center justify-between px-4">
                                    ...
                                    {!readOnly && (
                                        <button
                                            ...
                                            className="group px-6 py-3 bg-gradient-to-r from-emerald-600 to-green-500 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:shadow-lg hover:shadow-emerald-500/20 transition-all active:scale-95 flex items-center gap-2"
                                        >
                                ...
                                <div id="tasks-section" className="grid grid-cols-1 gap-4">
                                    {tasks.length === 0 ? (
                                        ...
                                        tasks.map((task) => (
                                            <div
                                                key={task.id}
                                                onClick={() => {
                                                    const url = readOnly ? `/student/tasks/${task.id}` : `/admin/groups/${group.id}/tasks/${task.id}`;
                                                    router.push(url);
                                                }}
                                                className="bg-white border border-slate-100 rounded-[1.5rem] p-4 group hover:border-emerald-500/20 hover:shadow-xl hover:shadow-emerald-500/10 hover:-translate-y-1 transition-all duration-300 cursor-pointer"
                                            >
                                                <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                                                    <div className="space-y-2 flex-1 min-w-0">
                                                        ...
                                                        {task.attachments && task.attachments.length > 0 && (
                                                            <div className="flex items-center gap-2 pt-1 overflow-x-auto pb-2 scrollbar-hide" onClick={(e) => e.stopPropagation()}>
                                                    ...
                                                    <div className="flex items-center gap-2 flex-shrink-0">
                                                        {(!readOnly && (currentUserRole === 'top_admin' || currentUser?.id === task.creator_id)) ? (
                                                            <>
                                                                <button
                                                            ...
                                                            readOnly && (
                                                                <button
                                                                    ...
                                                                    className="px-4 py-2 bg-slate-900 text-white rounded-xl font-black text-[9px] uppercase tracking-widest group-hover:bg-emerald-600 transition-all flex items-center gap-2"
                                                                >
                                                                    Submit Assignment
