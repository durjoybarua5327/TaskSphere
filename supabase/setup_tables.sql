...
-- Enable UUID extension (still needed for other IDs)
...
-- This assumes you have configured Supabase with the Clerk JWT Secret
-- and that auth.uid() returns the Clerk User ID (the 'sub' claim).
-- If not using JWTs, RLS will not work as expected via the API.
...
-- Note: The trigger to auto-create public.users from auth.users is REMOVED
-- because Clerk users aren't created in auth.users.
-- You must create the user in public.users via a Webhook (recommended) 
-- or inside your application logic (e.g. syncUser function) upon login.
